...
//for (obj of here_it_comes){
    //let make_p = document.createElement("h2")
   // let take_body = document.querySelector("body")
    //let edit = make_p.innerText = obj.title
...
//searchbar
let take_logo = document.getElementById("logo")
let take_menu = document.getElementById("menuu")
let take_windows = document.querySelector("#change_platfotm")
let take_search_bar = document.getElementById("search_bar")
let take_seach_button = document.getElementById("search_game")
let take_turns = 0
...
function start_search(){
  ...
  if (take_turns === 0) {
    ...
    take_windows.style.display = "none"
    take_logo.style.display = "none"
    take_menu.style.display = "none"
    take_seach_button.innerHTML = `<svg id="cancel_search" stroke="currentColor" fill="#04aad2" stroke-width="0" viewBox="0 0 512 512" height="70%" width="53%" xmlns="http://www.w3.org/2000/svg"><path d="M405 136.798L375.202 107 256 226.202 136.798 107 107 136.798 226.202 256 107 375.202 136.798 405 256 285.798 375.202 405 405 375.202 285.798 256z"></path></svg>`
    take_search_bar.style.display = "flex"
    take_turns += 1

  } else {

    take_windows.style.display = "block"
    take_logo.style.display = "block"
    take_menu.style.display = "block"
    take_search_bar.style.display = "none"
    take_seach_button.innerHTML = `<svg stroke="currentColor" fill="#04aad2" stroke-width="0" viewBox="0 0 512 512"  height="50%" width="40%" xmlns="http://www.w3.org/2000/svg"><path d="M505 442.7L405.3 343c-4.5-4.5-10.6-7-17-7H372c27.6-35.3 44-79.7 44-128C416 93.1 322.9 0 208 0S0 93.1 0 208s93.1 208 208 208c48.3 0 92.7-16.4 128-44v16.3c0 6.4 2.5 12.5 7 17l99.7 99.7c9.4 9.4 24.6 9.4 33.9 0l28.3-28.3c9.4-9.4 9.4-24.6.1-34zM208 336c-70.7 0-128-57.2-128-128 0-70.7 57.2-128 128-128 70.7 0 128 57.2 128 128 0 70.7-57.2 128-128 128z"></path></svg>`
...
let all_topics_taken = []

for (genres of here_it_comes){

    if (all_topics_taken.includes(genres.genre)){
      ...
      let take_allTopics = document.getElementById("all_topics")
      let edit = make_p.innerText = genres.genre
      console.log(all_topics_taken)

      take_allTopics.appendChild(make_p)
      all_topics_taken.push(genres.genre)
...
//show_games_sponsored
let all_sponsors_taken = []
for (imgs of here_it_comes){

  if (all_sponsors_taken.length > 5){
    ...
    let make_img = document.createElement("img")
    let make_src = make_img.setAttribute("src", imgs.thumbnail)
    let take_allTopics = document.getElementById("sponsored_games")
    console.log(all_sponsors_taken)

    take_allTopics.appendChild(make_img)
    all_sponsors_taken.push(imgs.thumbnail)
...
let turns_for_pictures = 0

function right_left(eve){

  let get_id_of = eve.getAttribute("id")
  let get_id_of_eve = get_id_of
  console.log(get_id_of_eve)
  if (get_id_of_eve === "right_arr" && turns_for_pictures < 7){
    turns_for_pictures += 1
    console.log(turns_for_pictures)
    let e = document.querySelector("#sponsored_games")
    e.scrollLeft += 700

  }
  else if(get_id_of_eve === "left_arr" && turns_for_pictures > 0){
    turns_for_pictures -= 1
    console.log(turns_for_pictures)
    let e = document.querySelector("#sponsored_games")
    e.scrollLeft -= 700
...
//show_games_shooter
let all_shooter_taken = []
let take_link_of_game = []
let game_pos = 0
for (shooter of here_it_comes){

  if (shooter.genre === "Shooter"){
    console.log(shooter)
    take_link_of_game.push(shooter.game_url)


    let make_img = document.createElement("img")
    let make_src = make_img.setAttribute("src", shooter.thumbnail)
    let make_src_value = make_img.setAttribute("valueshooter", game_pos)
    let make_src_onclick = make_img.setAttribute("onclick", "redirect_to_game(this)")
    let take_allTopics = document.getElementById("Shooteer")
    console.log(all_shooter_taken)

    take_allTopics.appendChild(make_img)
    game_pos++
    all_shooter_taken.push(imgs.thumbnail)
  ...
  console.log(take_link_of_game[0])


}

let turns_for_shooter = 0
let x_position = 0
//for topic_games arrows
function right_lefte(eve){

  let get_id_of = eve.getAttribute("id")
  let get_id_of_eve =get_id_of

  let get_value_of = eve.getAttribute("value")
  let get_value_of_eve = get_value_of
  console.log(typeof(get_value_of_eve))
  if (get_id_of_eve === "right_arr_shooter"){

    if (get_value_of_eve === "0"){

      console.log(turns_for_pictures)
      let e = document.querySelector("#Shooteer")
      e.scrollLeft += 650

    } else if(get_value_of_eve === "1"){
  ...
  else if(get_id_of_eve === "left_arr_shooter" ){
...
//show games MMORPG
let all_MMORPG_taken = []
let take_link_of_MMORPG = []
let MMORPG_pos = 0
for (MMORPG of here_it_comes){

  if (MMORPG.genre === "MMORPG"){
    console.log(shooter)
    take_link_of_MMORPG.push(MMORPG.game_url)


    let make_img = document.createElement("img")
    let make_src = make_img.setAttribute("src", MMORPG.thumbnail)
    let make_src_value = make_img.setAttribute("valueMMORPG", MMORPG_pos)
    let make_src_onclick = make_img.setAttribute("onclick", "redirect_to_game(this)")
    let take_allTopics = document.getElementById("MMOORPG")
    console.log(all_shooter_taken)

    take_allTopics.appendChild(make_img)
    MMORPG_pos++
    all_MMORPG_taken.push(MMORPG.thumbnail)
  ...
  console.log(take_link_of_MMORPG[0])
...
function redirect_to_game(this_selected){
  ...
  console.log(this_selected.closest("div"))
  let nearest_topic = this_selected.closest("div")
  console.log(nearest_topic.getAttribute("id"))

  if (nearest_topic.getAttribute("id") === "MMOORPG") {


    let get_value = this_selected.getAttribute("valueMMORPG")
    console.log(get_value)
    window.open(take_link_of_MMORPG[get_value], '_blank')
  

  } else if (nearest_topic.getAttribute("id") === "Shooteer") {
